...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var connection = new signalR.HubConnectionBuilder().withUrl("/updateIssueTLIPHub").build();
    ...
    connection.on("IssuedUpdate", function (message) {
        console.log("Received SignalR update:", message);
        fetchUpdatedEvents();
    });

    // Lắng nghe sự kiện xóa
    connection.on("EventIssuedDeleted", function (eventId) {
    ...
    */
    function fetchUpdatedEvents() {
        fetch('/TLIPWarehouse/GetUpdatedEventsIssued')
    ...
    */
            ...
            details.forEach(detail => {
                ...
                if (detail.Actuals && detail.Actuals.length > 0) {
                    detail.Actuals.forEach(actual => {
                        ...
                        const planStartTime = `${actualDateString}T${detail.PlanTimeIssued}`;
                        const [hours, minutes] = detail.PlanTimeIssued.split(':').map(Number);
        ...
        planDetails.forEach(detail => {
            ...
            if (!nextPlanStartDate || today < nextPlanStartDate) {
                ...
                while (startDate < endDate) {
                    ...
                    const start = `${dateString}T${detail.PlanTimeIssued}`;
                    const [hours, minutes] = detail.PlanTimeIssued.split(':').map(Number);
    ...
    function confirmActual(planDetailId, planDetailName) {
        ...
        fetch('/TLIPWarehouse/AddActualIssued', {
            method: 'POST',
            headers: {
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function () {
        if (deleteActualId) {
            fetch(`/TLIPWarehouse/DeleteActualIssued/${deleteActualId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
   ...
   */
    function checkForLateShipments(planDetails) {
        ...
        planDetails.forEach(detail => {
            const planTimeParts = detail.PlanTimeIssued.split(':');
